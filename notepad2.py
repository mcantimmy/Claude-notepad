...
class InteractiveNotepad:
    def __init__(self, root):
        ...
        self.root.geometry("1000x700")  # Increased default window size
        self.blocks = []
        self.create_widgets()
        self.set_style()
    ...
    def set_style(self):
        style = ttk.Style()
        style.theme_use("equilux")  # Using a modern dark theme
    ...
    def create_widgets(self):
        ...
        self.status_bar = ttk.Label(self.root, text="Ready", anchor=tk.W, background="#1e1e1e", foreground="#ffffff")
        self.status_bar.pack(side=tk.BOTTOM, fill=tk.X)

        self.root.protocol("WM_DELETE_WINDOW", self.on_closing)

    def create_menu_bar(self):
        menu_bar = tk.Menu(self.root, bg="#2e2e2e", fg="#ffffff")
...
class TextBlock:
    def __init__(self, parent, notepad):
        ...
        self.text = tk.Text(self.frame, height=5, wrap=tk.WORD, bg="#3e3e3e", fg="#ffffff", insertbackground="#ffffff")
...
class DrawingBlock:
    def __init__(self, parent, notepad):
        ...
        self.canvas = tk.Canvas(self.frame, width=400, height=200, bg='#3e3e3e', highlightthickness=0)
        self.canvas.pack()
    ...
    def paint(self, event):
        if self.last_x and self.last_y:
            self.canvas.create_line(self.last_x, self.last_y, event.x, event.y, fill='#ffffff', width=2, smooth=tk.TRUE, splinesteps=36)
            self.draw.line([self.last_x, self.last_y, event.x, event.y], fill='#ffffff', width=2)
        self.last_x = event.x
        self.last_y = event.y
        self.update_image()
...
class ClaudeBlock:
    def __init__(self, parent, notepad):
        ...
        self.delete_button = ttk.Button(self.frame, text="Delete", command=self.delete)
        self.delete_button.pack(side=tk.RIGHT, padx=(5, 0), pady=(5, 0))
